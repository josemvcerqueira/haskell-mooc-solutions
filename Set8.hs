...
blendColor (Color a b c) (Color d e f) = Color a' b' c'
  where
    avg :: Int -> Int -> Int
    avg x y = (x + y) `div` 2
    a' = avg a d
...
union (Shape s1) (Shape s2) = Shape s3
  where
    s3 :: Coord -> Bool
    s3 x = s1 x || s2 x

cut :: Shape -> Shape -> Shape
cut (Shape s1) (Shape s2) = Shape s3
  where
    s3 :: Coord -> Bool
    s3 x
      | s2 x = False
...
paintSolid color (Shape s) (Picture p1) = Picture f
  where
    f :: Coord -> Color
    f x
      | s x = color
...
paint (Picture p1) (Shape s1) (Picture p2) = Picture f
  where
    f :: Coord -> Color
    f x
      | s1 x = p1 x
...
class Transform t where
  ...
  blur4 :: t -> (Coord -> Color) -> Coord -> Color
  blur4 _ p c@(Coord x y) =
    let top = p $ Coord x (y - 1)
        left = p $ Coord (x - 1) y
        right = p $ Coord (x + 1) y
        bottom = p $ Coord x (y + 1)
        avg a b c d e = (a + b + c + d + e) `div` 5
        go (Color a1 b1 c1) (Color a2 b2 c2) (Color a3 b3 c3) (Color a4 b4 c4) (Color a5 b5 c5) = Color (avg a1 a2 a3 a4 a5) (avg b1 b2 b3 b4 b5) (avg c1 c2 c3 c4 c5)
     in go top left right bottom (p c)
...
instance Transform Blur where
  apply x (Picture p) = Picture $ blur4 x p
...
instance Transform BlurMany where
  apply (BlurMany x) (Picture p) = if x == 0 then Picture p else apply (BlurMany (x - 1)) (Picture $ blur4 (BlurMany 0) p)
