...
hSelectLines = go [] 1
  where
    go :: [String] -> Int -> Handle -> [Int] -> IO [String]
    go acc _ _ [] = return acc
    go acc i h ls@(x : xs) = do
      pred <- hIsEOF h
      let nextIndex = i + 1
      if pred
        then return acc
          ...
          y <- hGetLine h
          if i == x
            then go (acc <> [y]) nextIndex h xs
            else go acc nextIndex h ls
...
interact' f state = do
  x <- getLine
  let (pred, y, newState) = f (x, state)
  putStrLn y
  if pred then interact' f newState else return newState
