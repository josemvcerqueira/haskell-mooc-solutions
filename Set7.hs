...
data State = Start | Error | Finished | EggsAdded | OnlyFlourAdded | OnlySugarAdded | SugarAndFlourAdded | Mixed
  deriving (Eq, Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = EggsAdded
step EggsAdded AddFlour = OnlyFlourAdded
step OnlyFlourAdded AddSugar = SugarAndFlourAdded
step EggsAdded AddSugar = OnlySugarAdded
step OnlySugarAdded AddFlour = SugarAndFlourAdded
...
average (x :| xs) = go x 1 xs
  where
    go acc l [] = acc / l
    go acc l (x : xs) = go (acc + x) (l + 1) xs

------------------------------------------------------------------------------
-- Ex 5: reverse a NonEmpty list.

reverseNonEmpty :: NonEmpty a -> NonEmpty a
reverseNonEmpty y@(x :| []) = y
reverseNonEmpty (x :| xs) = go xs [x]
  where
    go [] acc = head acc :| tail acc
...
passwordAllowed p (ContainsSome c) = foldr (\x acc -> acc || x `elem` c) False p
...
evaluate (Literal x _) = x

render :: Arithmetic -> String
render (Literal _ y) = y
