...
readWords n = replicateM n getLine >>= \l -> return $ sort l
...
readUntil = go []
  where
    go :: [String] -> (String -> Bool) -> IO [String]
    go xs f = do
      x <- getLine
      let pred = f x
      if pred then return xs else go (xs <> [x]) f
...
countdownPrint n =
  when (n >= 0) (print n >> countdownPrint (n -1))
...
isums = go 0
  where
    ...
    go acc pred = do
      x <- getLine
      let parsedX = read x :: Int
      let y = acc + parsedX
      print y
      if pred == 1 then return y else go y $ pred - 1
...
whenM :: IO Bool -> IO () -> IO ()
whenM cond op = cond >>= \x -> when x op
...
while :: IO Bool -> IO () -> IO ()
while cond op = cond >>= \pred -> when pred $ op >> while cond op
