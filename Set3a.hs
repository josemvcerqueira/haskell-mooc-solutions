...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if maxA > maxB then a else b
  where
    maxA = measure a
    maxB = measure b
...
firstHalf :: [a] -> [a]
firstHalf x = if even l then take halfL x else take (halfL + 1) x
  where
    l = length x
    halfL = l `div` 2
...
joinToLength l xs = [k | x <- xs, y <- xs, let k = x ++ y, length k == l]
