  ...
  do
    ...
    if place `elem` s
      then return ()
        ...
        modify (place :)
        let (Just x) = lookup place maze
        if null x
          then return ()
          else mapM_ (visit maze) x
...
path maze place1 place2 = let (_, s) = runState (visit maze place1) [] in place2 `elem` s
...
instance Functor SL where
  -- implement fmap
  fmap f (SL g) = SL (\s -> let (a, b, c) = g s in (f a, b, c))
...
instance Monad SL where
  ...
  (SL g) >>= f = go g f
    where
      go :: (Int -> (a, Int, [String])) -> (a -> SL b) -> SL b
      go g' f' =
        SL
          ( \z ->
              let (a, b, c) = g' z
