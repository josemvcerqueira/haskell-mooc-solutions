...
statements xs ys = op1 <> op2
  where
    op1 = liftA2 (\x y -> x <> " is " <> y) xs ys
    op2 = (\x y -> x <> " is not " <> y) <$> xs <*> ys
...
validateAddress streetName streetNumber postCode = Address <$> checkedName <*> checkedNumber <*> checkedCode
  where
    check :: Bool -> String -> String -> Validation String
    check False message _ = invalid message
    check _ _ value = pure value
    allNumber :: String -> Bool
    allNumber = all isNumber
    checkedName :: Validation String
    checkedName = check (length streetName <= 20) "Invalid street name" streetName
    checkedNumber :: Validation String
    checkedNumber = check (allNumber streetNumber) "Invalid street number" streetNumber
    checkedCode :: Validation String
    checkedCode = check (length postCode == 5 && allNumber postCode) "Invalid postcode" postCode
...
boolOrInt x = checkBool xBool <|> checkInt xInt
  where
    xBool = readMaybe x :: Maybe Bool
    xInt = readMaybe x :: Maybe Int
    checkBool :: Maybe Bool -> Validation (Either Bool Int)
    checkBool Nothing = invalid "Not a Bool"
    checkBool (Just x) = pure $ Left x
    checkInt :: Maybe Int -> Validation (Either Bool Int)
    checkInt Nothing = invalid "Not an Int"
    checkInt (Just x) = pure $ Right x
...
normalizePhone x = checkLength *> checkDigits
  where
    x' = filter (not . isSpace) x
    checkLength = if length x' <= 10 then pure x' else invalid "Too long"
    checkDigits = traverse (\y -> if isNumber y then pure y else invalid $ "Invalid character: " <> [y]) x'
...
parseExpression x = if length x' /= 3 then invalid ("Invalid expression: " <> x) else checkedSign <*> (checkNumber (head x'') <|> checkVariable (head x'')) <*> (checkNumber (last x'') <|> checkVariable (last x''))
  where
    x' = words x
    sign = head $ tail x'
    x'' = [head x', last x']
    checkSign x
      | x == "+" = pure Plus
      | x == "-" = pure Minus
      | otherwise = invalid $ "Unknown operator: " <> x
    checkedSign = checkSign sign
    checkNumber x
      | all isNumber x = pure $ Number $ read x
      | otherwise = invalid $ "Invalid number: " <> x
    checkVariable x
      | length x == 1 && all isAlpha x = pure $ Variable $ head x
      | otherwise = invalid $ "Invalid variable: " <> x
...
instance Applicative Priced where
  pure = Priced 0
  liftA2 f (Priced a b) (Priced a' b') = Priced (a + a') (f b b')
