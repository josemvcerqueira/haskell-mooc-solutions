...
balance db account =
  do
    x <- query db balanceQuery [account] :: IO [[Int]]
    return $ (sum . Prelude.concat) x
...
parseCommand path
  | length path == 2 && head path == T.pack "balance" = Just $ Balance $ last path
  | length path == 3 && head path == T.pack "deposit" && validN path = Just $ Deposit ((head . tail) path) (go $ last path)
  | length path == 3 && head path == T.pack "withdraw" && validN path = Just $ Withdraw ((head . tail) path) (negate $ go $ last path)
  | otherwise = Nothing
  where
    go :: T.Text -> Int
    go x = case parseInt x of
      Just y -> y
      _ -> 0
    validN :: [T.Text] -> Bool
    validN = foldr (\x acc -> acc && C.isNumber x) True . T.unpack . last
...
perform db (Just x) = case x of
  Balance x -> T.pack . show <$> balance db x
  Deposit x y -> deposit db x y >> return (T.pack "OK")
  Withdraw x y -> deposit db x y >> return (T.pack "OK")
...
server db request respond =
  do
    x <- perform db $ parseCommand $ pathInfo request
    respond $ responseLBS status200 [] $ encodeResponse x
