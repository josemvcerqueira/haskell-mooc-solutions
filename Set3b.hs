...
sums i = sums' i 1 0
  where
    sums' i index prev = if index >= i then [prev + i] else prev + index : sums' i (index + 1) (prev + index)
...
indexDefault xs i def = if i < 0 then def else indexDefault' xs i def 0
  where
    ...
    indexDefault' (x : xs) i def acc = if acc >= i then x else indexDefault' xs i def (acc + 1)
...
sumsOf xs = sums' xs 0
  where
    sums' [] _ = []
    sums' z@(x : xs) acc = if acc == 0 then x : sums' xs x else x + acc : sums' xs (x + acc)
...
merge :: [Int] -> [Int] -> [Int]
merge xs ys = merge' xs ys 0
  where
    merge' [] ys _ = ys
    merge' xs [] _ = xs
    merge' list1@(x : xs) list2@(y : ys) prev = if x > y then y : merge' list1 ys y else x : merge' xs list2 x
...
mymaximum bigger initial xs = initial' bigger xs initial
  where
    initial' _ [] acc = acc
    initial' bigger (x : xs) acc = if bigger x acc then initial' bigger xs x else initial' bigger xs acc
...
maybeMap f (x : xs) = maybeMap' f xs (f x)
  where
    maybeMap' f [] (Just z) = [z]
    maybeMap' f [] Nothing = []
    maybeMap' f (x : xs) (Just z) = z : maybeMap' f xs (f x)
