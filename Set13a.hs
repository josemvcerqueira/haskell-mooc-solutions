...
split = (\x -> if length x == 2 then Just (head x, last x) else Nothing) . words
...
checkNumber (x, y)
  | hasNumber x || hasNumber y = Nothing
  | otherwise = Just (x, y)
  where
    hasNumber :: String -> Bool
    hasNumber = foldr (\s acc -> acc || not (isAlpha s)) False
...
winner scores player1 player2 =
  do
    ...
    go player1Score player2Score player1 player2
  where
    go :: Int -> Int -> String -> String -> Maybe String
    go score1 score2 p1 p2
...
selectSum xs is =
  do
    i <- safeIHead is $ length xs
    let num = xs !! i
    x <- selectSum xs $ tail is
    return $ num + x
  where
    safeIHead :: (Num a, Ord a) => [a] -> a -> Maybe a
    safeIHead [] _ = Nothing
    safeIHead (x : _) l
      | x < 0 = Nothing
      | x >= l = Nothing
...
countAndLog :: (Show a, Num a) => (a -> Bool) -> [a] -> Logger Int
...
balance accountName = BankOp (\(Bank x) -> (find x accountName, Bank x))
  where
    find :: Map.Map String Int -> String -> Int
    find m s =
      let x = Map.lookup s m
          get (Just x) = x
          get Nothing = 0
       in if x == Nothing then 0 else get x
...
count x = modify $ go False
  where
    go False [] = [(x, 1)]
    go True [] = []
    go b ((k, t) : ys)
      | k == x = (k, t + 1) : go True ys
      | otherwise = (k, t) : go b ys
