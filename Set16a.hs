...
sumIsLength input output = sum' === length input
  where
    sum' = foldr (\(_, y) accum -> accum + y) 0 output
...
inputInOutput input output = forAll (elements input) check
  where
    check x = foldr (\(x', _) acc -> acc || x' == x) False output
...
outputInInput input output = forAll (elements output) check
  where
    check (a, b) = let result = foldr (\x acc@(a', b') -> if a == x then (a', b' + 1) else acc) (a, 0) input in result == (a, b)
...
frequenciesProp :: (String -> [(Char, Int)]) -> NonEmptyList Char -> Property
frequenciesProp freq (NonEmpty input) = let output = freq input in conjoin [outputInInput input output, inputInOutput input output, sumIsLength input output]
...
instance Arbitrary Arg where
  arbitrary = do
    num <- elements [0 .. 10]
    letter <- elements ['a', 'b', 'c', 'x', 'y', 'y']
    oneof [return $ Number num, return $ Variable letter]

instance Arbitrary Expression where
  arbitrary = do
    v1 <- arbitrary :: Gen Arg
    v2 <- arbitrary :: Gen Arg
    oneof [return $ Plus v1 v2, return $ Minus v1 v2]
