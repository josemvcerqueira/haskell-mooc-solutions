...
treeMax Empty = 0
treeMax x = treeMax x 0
  where
    treeMax Empty acc = acc
    treeMax (Node x left right) acc = if acc > x then max (treeMax left acc) (treeMax right acc) else max (treeMax left x) (treeMax right x)
...
search val tree = search' val tree []
  where
    ...
    search' val (Node x left right) acc
      | val == x = Just acc
      | otherwise =
        let leftResult = search' val left (acc ++ [StepL])
            pred = isNothing leftResult
         in if pred then search' val right (acc ++ [StepR]) else leftResult
