...
count pred = foldr (\x acc -> if pred == x then acc + 1 else acc) 0
...
inBoth f1 f2 = foldr (\x acc -> if foldr (\y acc -> acc || y == x) False f2 then x : acc else acc) [] f1
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode x y ls) = f x $ f y $ foldr f acc ls
