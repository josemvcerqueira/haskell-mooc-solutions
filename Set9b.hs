...
prettyPrint s = foldr (\(row, col) -> addQueen ((((row - 1) * (s + 1)) + col) - 1)) (makeBoard s)
  where
    makeDot :: Int -> String
    makeDot 0 = ['\n']
    makeDot n = '.' : makeDot (n -1)
    row :: String
    row = makeDot s
    makeBoard :: Int -> String
    makeBoard 0 = []
    makeBoard n = row <> makeBoard (n -1)
    addQueen :: Int -> String -> String
    addQueen n ls = a ++ ('Q' : b) where (a, _ : b) = splitAt n ls
...
danger c = foldr (\cTest acc -> acc || go c cTest) False
  where
    go :: Candidate -> Candidate -> Bool
    go c1 c2 = sameRow c1 c2 || sameCol c1 c2 || sameDiag c1 c2 || sameAntidiag c1 c2
...
prettyPrint2 size queens = go $ makeBoard (size * size) (1, 1)
  where
    ...
    nextCoord (row, col)
      ...
      | col == size = (row + 1, 1)
      | otherwise = (row, col + 1)
    makeBoard :: Size -> Coord -> [Coord]
    makeBoard 0 _ = []
    makeBoard s acc = acc : makeBoard (s - 1) (nextCoord acc)
    isDanger :: Coord -> Bool
    isDanger = flip danger queens
    addBreak :: Coord -> String -> String
    addBreak (_, c) acc = if c == size then acc <> "\n" else acc
    fn :: String -> Coord -> String
    fn acc x@(c, r)
      | x `elem` queens = addBreak x $ acc <> "Q"
      | isDanger x = addBreak x $ acc <> "#"
      | otherwise = addBreak x $ acc <> "."
    go :: [Coord] -> String
    go = foldl' fn ""
...
fixFirst n s = go $ head s
  where
    stackTail :: Stack
    stackTail = tail s
    go :: Candidate -> Maybe Stack
    go candidate@(r, c)
      | c > n = Nothing
      | not $ danger candidate stackTail = Just $ candidate : stackTail
      | danger candidate stackTail = go $ nextCol candidate
      | otherwise = Nothing
...
continue [] = []
continue s@(x : _) = nextRow x : s

backtrack :: Stack -> Stack
backtrack [] = []
backtrack s@(x : xs) = fstTail : remaining
  where
    tailList :: Stack
    tailList = tail s
    remaining :: Stack
    remaining = tail tailList
    fstTail :: Coord
    fstTail = nextCol $ head tailList
